...
void jogar();
bool procLetra(char letra, char palavraEdit[201], char *esconde, char palavra[201]); //proc de procurar

unsigned int palavraso; //para ver as letras
...
void jogar(){
    ...
    bool achou = false;
    unsigned char letra;
    unsigned letDig[100]={'\0'};
    unsigned char palavraEdit[201];
    unsigned char esconde[201];
    dica gua;
    FILE *arq;
    sorteio();
    arq = abrirArq("palavras.fc", "r"); //r = read
...
} // fecha na outra estrutura do for
    esconde[i] = '\0';
    j = 0;
    while(fim == false){
        system('cls');
...
void adicionarDica(){
    ...
    do{
        system("cls");
        printf("\nDIGITE UMA DICA DE ATE 201 CARACTERES");
        fflush(stdin);
        scanf("%200[^\n]", guarda.dica);
...
void adicionarPalavra(){
    ...
    do{
     ...
     fflush(stdin);
     scanf("%200[^\n]",guarda.palavra);
     printf("DIGITE A DIGA: ");
     fflush(stdin);
     scanf("%200[^\n]", gua.dica);
     printf("DESEJA CONTINUAR COM A PALAVRA %S? (S/N) ", guarda.palavra);
     fflush(stdin);
     resp = getchar();
    }while(resp != 'S' && resp != 's');
    ...
    if(erro == true){
        printf("\nPALAVRA JA EXISTE!");
        printf("\nAPERTA UMA TECLA PARA VOLTAR AO MENU... ");
        fflush(stdin);
        resp = getchar();
        return;
    }
...
FILE* abrirArq(char nome[], char modo[]){
    ...
    if(arq == NULL){
        printf("\nNAO FOI POSSIVEL ABRIR O ARQUIVO %d\n", nome);
    }
    return arq;
}
